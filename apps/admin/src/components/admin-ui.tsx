...
// Shared admin building blocks so Dashboard, Menu Manager and Restaurant
// Settings share one visual language (navy/orange, soft white cards).

export const NAVY = '#16233C';
export const NAVY_PANEL = '#1E2D4A';
...
export function PageHero({ title, subtitle, actions, status }: PageHeroProps) {
  return (
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h1 className="text-3xl font-bold text-[#1E2D4A]">{title}</h1>
  ...
  return (
    <section className={`rounded-2xl border border-gray-100 bg-white p-6 shadow-sm ${className ?? ''}`}>
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-base font-semibold text-[#1E2D4A]">{title}</h2>
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${iconClass ?? 'bg-orange-50 text-primary'}`}>
          <Icon className="h-4 w-4" />
        </span>
      </div>
      <div className="mt-3 text-3xl font-bold text-[#1E2D4A]">{value}</div>
      {hint ? <p className="mt-1 text-xs text-muted-foreground">{hint}</p> : null}
    </div>
  );
}

// Small uppercase field label used across the settings forms.

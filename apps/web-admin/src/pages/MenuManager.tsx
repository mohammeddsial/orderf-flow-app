  ...
  const saveItem = async () => {
    ...
    try {
      ...
      setDeleteResult({
        open: true,
        title: '❌ Error',
        description: 'Could not reach the backend on :4000.',
        variant: 'error',
      });
    }
  };
      ...
      complete: async (result) => {
        ...
        for (const row of rows) {
          ...
          try {
            ...
            console.warn('⚠️ Invalid modifiers JSON, using empty array', row);
            modifiers = [];
          }
    ...
    <Layout title="Menu Manager" breadcrumb="Menu Manager" searchPlaceholder="Search menu items...">
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center text-xl">Delete Item?</DialogTitle>
      ...
      <ResultDialog
        ...
        onOpenChange={(open) => setDeleteResult((prev) => ({ ...prev, open }))}
